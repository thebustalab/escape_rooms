...
- **Paired scenarios are a pre/post test — match the question STYLE across the pair (2026-07-18).**
  ...
  pair. (**`trees` is COMPLETE** — all 8 rooms built, audited 2026-08-27, `status: "ready"`, student-facing.
  `egypt` is still in development, 11 rooms; keep its question style mirrored to the shipped trees ladder.)
...
| 15 | **Grid-select (matrix selector)** — an N-col × M-row button grid; select one cell per column; the selection vector checks against a key → `solveRoom` (ungraded escape gate; sibling of `lock`/`ledger`, out of the codec) | Myst/Riven combination panels | Recognise/produce a **mapping**: which grouping made a display (trees), or which trait per queued verb (egypt). **Egypt active variant:** collect image-cards → **group-and-summarise** them by visual traits via a symbol-queue, entering results on the grid — a *data-free* active echo of `group_by \|> summarise` | **BUILT 2026-07-26, SHIPPED 2026-08-27** (`grid` hotspot in `shared/pano-player.js` — items×buckets matrix, one bucket per item, checks against `answer`; ungraded like `lock`). **Browser test now DONE** — the `trees` row in `tests/e2e/full_playthrough.spec.js` clicks the real 3×3 panel in headless Chromium and asserts the solve. **First SHIPPED consumer: `wrangling/trees` vault** (□→colour / ○→size / △→shape; `status: "ready"`). Note the shipped shape: the grid does **NOT** carry `endsEscape` — it unseals a `door` hotspot that does, so the player walks through the opened gate rather than the finish screen firing under them (Lucas, 2026-08-27). Still specced for `comparing_means/squirrel` roost (4 kinds × 3 height-tiers) + `wrangling/egypt`. |
...
| 20 | **Observed-graph relay (survey by eye, then re-address around an outage)** — the player never receives the network as a table: they collect it by **looking**, one **image** sighting-card per place they stand, laid out on the notebook's draggable board until the graph takes shape. Then a node **goes out** and the escape asks them to get a message to a post they cannot see, around the gap. Three properties make it work: an edge is a **perceivable relation** ("I can see you"); the network is **derived from the terrain by real line of sight**, so every missing edge has a named mountain responsible and nothing is arbitrary; and the fire is only a **summons** — content rides on flags read through the glass, which is what lets a player at one post address a message at all rather than implausibly commanding distant stations to light in sequence (the historical optical telegraph). | Riven/Myst "the world reveals itself as you understand it"; the Obra Dinn book filled in by observation; Chappe semaphore | **Relational networks (Part 2), data-free** — connectivity and reachability under **node removal**, re-posed on in-world props instead of the CSV, which is the boss's move performed rather than repeated. The graded dataset is a **different network about different entities**, so the escape cannot be short-cut from the file | **Have** — composes existing pieces, no engine work: **#15 grid-select** (message slots x posts) + **image pickups on the collected-items board**. First consumer: `networks/beacons` ("Line of Sight"). **Rewritten 2026-08-19:** the original had the WEATHER change the edge set (fog drowning the low ground) — that version is RETIRED because the surviving posts formed a bare path, so the answer could be read off the picture with none of the survey. Terrain occlusion is permanent and legible; weather is drama. See `rooms/networks/beacons/notes.md` -> *Escape redesign*. |
